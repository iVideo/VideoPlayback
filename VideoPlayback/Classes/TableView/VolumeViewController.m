...
#define dataURL @"http://agiledev.bgsu.edu/~tdu/volume.php"
...
@synthesize json, volumeArray, myTableView, selectedJournalID;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"showIssue"]) {
        ...
        self.selectedVolume = [[NSString alloc] init];
        MESVolume *tempvolume = [volumeArray objectAtIndex:indexPath.row];
        
        destViewController.selectedVolume = tempvolume.Volume;
...
{
    static NSString *CellIdentifier = @"volumeCell";
    ...
    MESVolume * currentVolume = [volumeArray  objectAtIndex:indexPath.row];
    
    cell.textLabel.text = currentVolume.Volume ;
...
- (void) retrieveData{
    ...
    [manager POST:dataURL parameters:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for (int i = 0; i < [responseObject count]; i++) {
            ...
            MESVolume *myVolume = [[MESVolume alloc] initWithVolume: jvolume];
            
            //Add our Article object to Article array
            [volumeArray addObject: myVolume];
